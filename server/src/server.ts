...
const DEFAULT_TOTAL_DAYS: number = 24;
const MONGODB_URI: string = process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017';
const DB_NAME: string = process.env.DB_NAME || 'countdown24';

let client: MongoClient | null = null;
let database: Db | null = null;
let Users: Collection | null = null;
let Countdowns: Collection | null = null;
let Assignments: Collection | null = null;
let Tokens: Collection | null = null;
let CountdownDays: Collection | null = null;
...
async function issueToken(userId: string): Promise<{ token: string; expiresAt: number }> {
...
function addDays(date: string | Date, days: number): Date | null {
...
function normalizeDate(value: string | undefined, fallback?: string): string | null {
...
function requireRole(role: 'creator' | 'receiver') {
...
const asyncHandler = (handler: (req: Request, res: Response, next: NextFunction) => Promise<any>) => (req: Request, res: Response, next: NextFunction) => {
